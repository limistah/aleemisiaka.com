...
To you, the query reads: _Select the first child of every `P` tag that has a class name of `paragraph-1`,_ that is not correct!
...
In addition, the only child of an element, empty elements, any element that is a child to an element but existing at an index, and more are stylable, which is powerful!
