...
Funny enough, some handlers for some events can get so large. So, what do we do?

This is what we are trying to solve in this post. Firstly, let's create a folder for our events and add an `index.js` file into it.

```bash
mkdir src/events
touch ./src/events/index.js
```