...
I have a limitation for the menu to be deeply nested not more than two steps, so in this case, I the loops that I write won't have to be too recursive -  which if not properly handled could create havoc.

## Model/Database

In my database, I have the menu store as a flat document which each of them having their own information. The menu model has the following properties:
...
This sorting onl compares the weight of each menu item, these weights are expected to go in ascending order, hence we can use `a.weight > b.weight` so that we can have lowest number comes first ahead of larger number
