...
In this post, I will demonstrate how to create an ediable component. The component would be able to use different form fields and notify the parent if any change has been made. That said, Let's move....

To start with, I will define the usage that I want for my component. In my own case, I would love the component to be called like this:
...
Awesome, so, at this point, I was able to toggle between two modes, but nothing happens and nothing is rendered, yet. To move forward, I had to display the text which would be received by the `InplaceEdit` component and passed to the `TextDisplay` component as props. The below code is what I implemented for this phase:
...
Great! I was glad when I got to this point. I am sure you are as well, at least, something is getting rendered.

It is very correct if you guessed that the TextDisplay component will not change its state afterwards, and bulk of work is around the EditDisplay component. So, sorry, I won't be adding that style that you are thinking.
...
While the `SelectInput`, I defined it to look like this:
```js
